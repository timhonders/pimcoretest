...
class KegaAmqp_Amqp_AbstractChannel {
    ...
    function next_frame()
    {
        if($this->debug)
        {
          KegaAmqp_Amqp_Tools::debug_msg("waiting for a new frame");
        }
    ...
    function wait_content()
    {
        ...
        if($this->auto_decode and isset($msg->content_encoding))
        {
            try
            {
              ...
              if($this->debug)
              {
                KegaAmqp_Amqp_Tools::debug_msg("Ignoring body decoding exception: " . $e->getMessage());
              }
            }
        }
    ...
    public function wait($allowed_methods=NULL)
    {
        if($allowed_methods)
        {
          if($this->debug)
          {
            KegaAmqp_Amqp_Tools::debug_msg("waiting for " . implode(", ", $allowed_methods));
          }
        }
        else
        {
          if($this->debug)
          {
            KegaAmqp_Amqp_Tools::debug_msg("waiting for any method");
          }
        }
        ...
        foreach($this->method_queue as $qk=>$queued_method)
        {
          if($this->debug)
          {
            KegaAmqp_Amqp_Tools::debug_msg("checking queue method " . $qk);
          }
            ...
            if($allowed_methods==NULL || in_array($method_sig, $allowed_methods))
            {
                ...
                if($this->debug)
                {
                  KegaAmqp_Amqp_Tools::debug_msg("Executing queued method: $method_sig: " . KegaAmqp_Amqp_AbstractChannel::$GLOBAL_METHOD_NAMES[KegaAmqp_Amqp_Tools::methodSig($method_sig)]);
                }
        ...
        while(true)
        {
            ...
            $args = new KegaAmqp_Amqp_Reader(substr($payload,4));

            if($this->debug)
            {
              KegaAmqp_Amqp_Tools::debug_msg("> $method_sig: " . KegaAmqp_Amqp_AbstractChannel::$GLOBAL_METHOD_NAMES[KegaAmqp_Amqp_Tools::methodSig($method_sig)]);
            }


            if(in_array($method_sig, KegaAmqp_Amqp_AbstractChannel::$CONTENT_METHODS))
            ...
            if($allowed_methods==NULL ||
               ...
               in_array($method_sig,KegaAmqp_Amqp_AbstractChannel::$CLOSE_METHODS))
            {
            ...
            if($this->debug)
            {
              KegaAmqp_Amqp_Tools::debug_msg("Queueing for later: $method_sig: " . KegaAmqp_Amqp_AbstractChannel::$GLOBAL_METHOD_NAMES[KegaAmqp_Amqp_Tools::methodSig($method_sig)]);
            }
