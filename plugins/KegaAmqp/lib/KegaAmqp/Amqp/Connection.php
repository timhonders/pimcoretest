...
class KegaAmqp_Amqp_Connection extends KegaAmqp_Amqp_AbstractChannel
{
                                ...
                                $context = null)
    {
        ...
        if($user && $password)
        {
            $login_response = new KegaAmqp_Amqp_Writer();
        ...
        $d = KegaAmqp_Amqp_Connection::$LIBRARY_PROPERTIES;
        while(true)
        {
            $this->channels = array();
            ...
            $this->input = new KegaAmqp_Amqp_Reader(null, $this->sock);

            $this->write(KegaAmqp_Amqp_Connection::$AMQP_PROTOCOL_HEADER);
            ...
            if($this->debug)
            {
              KegaAmqp_Amqp_Tools::debug_msg("closing socket");
            }
    ...
    protected function write($data)
    {
        if($this->debug)
        {
          KegaAmqp_Amqp_Tools::debug_msg("< [hex]:\n" . KegaAmqp_Amqp_Tools::hexdump($data, $htmloutput = false, $uppercase = true, $return = true));
        }
    ...
    {
        if($args instanceof KegaAmqp_Amqp_Writer)
            $args = $args->getvalue();
        ...
        if($this->debug)
        {
          KegaAmqp_Amqp_Tools::debug_msg("< " . KegaAmqp_Amqp_Tools::methodSig($method_sig) . ": " . KegaAmqp_Amqp_AbstractChannel::$GLOBAL_METHOD_NAMES[methodSig($method_sig)]);
        }
    ...
    public function channel($channel_id=NULL)
    {
        ...
        return new KegaAmqp_Amqp_Channel($this->connection, $channel_id);
    }
    ...
    protected function _close($args)
    {
        ...
        throw new KegaAmqp_Amqp_Exceptions_ConnectionException($reply_code, $reply_text, array($class_id, $method_id));
    }
    ...
    protected function open_ok($args)
    {
        ...
        if($this->debug)
        {
          KegaAmqp_Amqp_Tools::debug_msg("Open OK! known_hosts: " . $this->known_hosts);
        }
    ...
    protected function redirect($args)
    {
        ...
        if($this->debug)
        {
          KegaAmqp_Amqp_Tools::debug_msg("Redirected to [". $host . "], known_hosts [" . $this->known_hosts . "]" );
        }
        return $host;
    }
    ...
    protected function start($args)
    {
        ...
        if($this->debug)
        {
          KegaAmqp_Amqp_Tools::debug_msg(sprintf("Start from server, version: %d.%d, properties: %s, mechanisms: %s, locales: %s",
                            $this->version_major,
                            $this->version_minor,
